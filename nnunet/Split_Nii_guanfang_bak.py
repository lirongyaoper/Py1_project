...
folder_path = r'D:\Data\test'   # nii_gz 文件夹位置
nii_gzs = get_subfolder_names(folder_path) # 获取所有nii_gz文件
for nii_gz in nii_gzs:
    a = nii_gz[:-7]
    if a == "lung_vessel":   # 只拆分这个nii.gz
        nii_gz_path = os.path.join(folder_path, nii_gz)
        decompress_nifti_gz(nii_gz_path)  # 解压nii.gz为nii
        os.remove(nii_gz_path)  # 删除nii.gz
        split_labels(nii_gz_path, folder_path)  # 拆分
